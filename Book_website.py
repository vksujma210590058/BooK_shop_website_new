...
st.title(":red[Vikas Book Shop At Paharia Varanasi]")
with st.container(height=300,border=True):
    answer=st.radio("Do You Want To Make YOUR website",["Yes","No"],index=None)
    con_num=st.number_input("Contact number ",step=1)
    # answer_list=[]
    # data_customer={"answer":[],"contact_number":[]}
    # data_customer["answer"]=
    if st.button(label="submit"):
        # st.write(con_num)
        # st.write(answer)
        st.write(":green[Form submitted]")
        import csv
        new_data_cust=[answer,con_num]
        file_path="customer_detail.csv"
        with open(file_path,mode="a",newline="")as file:
            writer=csv.writer(file)
            writer.writerow(new_data_cust)
...
# Run the function to display images
    ...
    with tab1.expander(":red[Search More Books]"):
        ...
        def generate_tab1_data():
            # Simulating the generation of tab1_data
            import time
            # time.sleep(5)  # Simulating a time-consuming task
            tab1_data = tab1_table
            return tab1_data

        if st.toggle(label=":red[Click me to see All Books of Upsc]", key="asdf"):
            st.text("Processing... Please wait.")

            # Create a variable to store the generated data
            tab1_data = None

            # Define a function that generates the data in a separate thread
            def generate_data_thread_func():
                global tab1_data
                tab1_data = generate_tab1_data()

            # Create and start the thread
            data_thread = threading.Thread(target=generate_data_thread_func)
            data_thread.start()
    ...
    with st.expander(label="I"):
        df_cust_detail=pd.read_csv("customer_detail.csv")
        me_vikas_code=st.text_input("vikas")
        if me_vikas_code=="210590058":
            st.write(df_cust_detail)
        me_vikas_code_delete=st.text_input("vikas go row.com ")
        if me_vikas_code_delete=="9936328404":
            import csv
            new_data_cust=["answer","contact"]
            file_path="customer_detail.csv"
            with open(file_path,mode="w",newline="")as file:
                writer=csv.writer(file)
                writer.writerow(new_data_cust)
            # ired background color
...
with tab2:
    ...
    with tab2.expander(":red[Search More Books]"):
        if st.toggle("Click me to see All Books Ssc"):
            st.write(tab2_table)
...
with tab3:
    ...
    with tab3.expander(":red[Search More Books]"):
        if st.toggle("Click me to see All Books Defence"):
            st.write(tab3_table)
...
with tab4:
    ...
    with tab4.expander(":red[Search More Books]"):
        if st.toggle("Click me to see All Books other"):
            st.write(tab4_table)
